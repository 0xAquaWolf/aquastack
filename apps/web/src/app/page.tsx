      ...
      <main className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          {quests?.map((quest: Quest) => (
            <div key={quest.id} className="p-4 bg-card rounded-lg shadow-md border">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold">{quest.title}</h3>
                  <p className="text-muted-foreground">{quest.description}</p>
                  <span className={`inline-block px-2 py-1 text-xs rounded ${
                    quest.status === 'completed' ? 'bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-200' :
                    quest.status === 'in_progress' ? 'bg-yellow-200 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-200' :
